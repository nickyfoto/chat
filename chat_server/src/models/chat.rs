...
impl AppState {
    pub async fn create_chat(&self, input: CreateChat, ws_id: u64) -> Result<Chat, AppError> {
        ...
        if len > 8 && input.name.is_none() {
            return Err(AppError::CreateChatError(
                "Group chat with more than 8 members mut have a name".to_string(),
            ));
        }

        // verify if all members exist
        let members = self.fetch_chat_user_by_ids(&input.members).await?;
        if members.len() != len {
            return Err(AppError::CreateChatError(
...
mod tests {
    ...
    async fn create_single_chat_should_work() -> Result<()> {
        ...
        let input = CreateChat::new("", vec![1, 2], false);
        let chat = state
            .create_chat(input, 1)
            .await
    ...
    async fn create_public_named_chat_should_work() -> Result<()> {
        let (_tdb, state) = AppState::new_for_test().await?;
        let input = CreateChat::new("test", vec![1, 2, 3], true);
        let chat = state
            .create_chat(input, 1)
            .await
            .expect("create chat failed");
